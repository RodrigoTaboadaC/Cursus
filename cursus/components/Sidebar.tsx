...
export default function Sidebar() {
  ...
  const link = (href: string, label: string, badge?: number) => {
    ...
    return (
      <a href={href} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors ${active ? "text-blue-600 bg-blue-50 font-medium" : "text-gray-500 hover:bg-gray-50"}`}>
        {label}
  ...
  return (
    <aside className="w-52 min-h-screen bg-white border-r border-gray-100 flex flex-col py-4">
      ...
      <nav className="flex flex-col gap-1 px-2">
        ...
        {link("/tareas", "Mis tareas", 4)}
        {link("/evaluaciones", "Evaluaciones", 3)}
        {link("/grupos", "Grupos")}
      ...
      <div className="mt-auto mx-3 bg-amber-50 rounded-lg p-3">
        ...
        <p className="text-xs text-amber-600 mt-0.5">Juega hoy para no perderla</p>
      </div>
    </aside>
  )
}